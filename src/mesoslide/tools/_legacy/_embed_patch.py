"""Deprecated: bespoke sdata-schema patch embedding (Vision/SAE/KMeans).

Superseded, for the vision foundation-model branch, by
`mesoslide.tools._embed_patch.embed_patch`, which consumes ezslide/wsidata
slides and lazyslide-models' MODEL_REGISTRY. Kept here unmodified (aside from
the embedder import path) for the SAE and KMeans branches and for existing
callers (`mesoslide.tools.segmenters._adaptive_sampling`) that still operate
on the old `{image}_{point}_{patch}` AnnData table schema.
"""
